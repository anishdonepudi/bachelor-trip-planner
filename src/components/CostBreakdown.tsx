...
import { CostBreakdown as CostBreakdownType, SearchMode } from "@/lib/types";
...
export function CostBreakdownTable({ perCityCosts, totalGroupCost, searchMode }: CostBreakdownProps) {
  return (
    <section>
        ...
        {perCityCosts.map((cost) => (
          <div key={cost.city} className="rounded-lg border border-[var(--border-default)] bg-[var(--surface-1)] p-3">
            ...
            <div className="flex items-center gap-2 text-xs text-[var(--text-3)]">
              ...
              {searchMode !== "stays" && cost.fallbackCategory && (
              ...
              {searchMode === "both" && <span className="text-[var(--text-3)]">+</span>}
              {searchMode !== "flights" && (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-[var(--surface-1)] text-[10px] text-[var(--text-3)] uppercase tracking-wider font-heading">
              ...
              {searchMode !== "stays" && <th className="text-right py-2.5 px-3 font-semibold">Flight</th>}
              {searchMode !== "flights" && <th className="text-right py-2.5 px-3 font-semibold">Stay</th>}
          ...
          <tfoot>
            <tr className="border-t-2 border-[var(--border-hover)] bg-[var(--surface-1)]">
              <td colSpan={searchMode === "both" ? 3 : 2} className="py-2.5 px-3 text-xs uppercase text-[var(--text-2)] font-heading font-semibold tracking-wider">
                Group Total
              </td>
