  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        {/* Tabs */}
        <div className="flex gap-0.5 p-0.5 rounded-md bg-[var(--surface-1)] border border-[var(--border-default)] shrink-0 mt-3">
          <button
            onClick={() => setActiveTab("group")}
            className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 rounded text-xs font-medium transition-all duration-150 ${
              activeTab === "group"
          ...
          >
            ...
            {excludedDates.length > 0 && (
              <span className="px-1 py-0.5 rounded bg-[var(--red-soft)] text-[var(--red)] text-[9px] font-bold">{excludedDates.length}</span>
            )}
          </button>
        </div>

        {/* Content */}
        <div className="flex-1 overflow-y-auto mt-3 min-h-0 pr-2 scrollbar-thin">
          {activeTab === "group" ? (
            <div className="space-y-2">
              {/* Destination */}
              <div className="p-3 rounded-md bg-[var(--surface-1)] border border-[var(--border-default)] space-y-2 opacity-50">
                <div>
                  <label className="text-[10px] font-heading font-semibold text-[var(--text-3)] uppercase tracking-wider">Destination</label>
                  <div className="mt-1 flex gap-2">
                    <input type="text" value={destinationAirport} disabled
                      className="w-16 bg-[var(--surface-2)] border border-[var(--border-default)] rounded-md px-2 py-1.5 text-sm text-[var(--text-1)] font-mono cursor-not-allowed" />
                    <input type="text" value={destinationCity} disabled
                      className="flex-1 bg-[var(--surface-2)] border border-[var(--border-default)] rounded-md px-2 py-1.5 text-sm text-[var(--text-1)] cursor-not-allowed" />
                  </div>
                </div>
                <p className="text-[10px] text-[var(--text-3)]">Destination editing coming soon</p>
              </div>

              {/* Divider */}
              <div className="flex items-center gap-2 py-1">
                <div className="flex-1 h-px bg-[var(--border-default)]" />
                <span className="text-[10px] font-heading font-semibold text-[var(--text-3)] uppercase tracking-wider">Cities</span>
                <div className="flex-1 h-px bg-[var(--border-default)]" />
              </div>

              {/* City rows */}
              {cities.map((city, i) => (
                <div key={i} className="group relative p-2.5 rounded-md bg-[var(--surface-1)] border border-[var(--border-default)] hover:border-[var(--border-hover)] transition-colors duration-150">
                  <div className="flex items-start gap-2.5">
                    <span className="text-[10px] font-mono font-semibold text-[var(--text-3)] w-4 text-right mt-2 shrink-0">{i + 1}</span>
                    ...
                    <div className="flex items-center shrink-0">
                      <button onClick={() => updateCity(i, "people", Math.max(1, city.people - 1))}
                        className="w-6 h-6 rounded-l-md bg-[var(--surface-2)] border border-[var(--border-default)] flex items-center justify-center text-[var(--text-2)] hover:text-[var(--text-1)] hover:bg-[var(--surface-3)] transition-colors duration-150">
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 12H4" />
                        </svg>
                      </button>
                      <div className="w-8 h-6 bg-[var(--surface-2)] border-y border-[var(--border-default)] flex items-center justify-center">
                        <span className="text-xs font-semibold font-mono tabular-nums text-[var(--text-1)]">{city.people}</span>
                      </div>
                      <button onClick={() => updateCity(i, "people", city.people + 1)}
                        className="w-6 h-6 rounded-r-md bg-[var(--surface-2)] border border-[var(--border-default)] flex items-center justify-center text-[var(--text-2)] hover:text-[var(--text-1)] hover:bg-[var(--surface-3)] transition-colors duration-150">
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button onClick={() => removeCity(i)}
                      className="p-1 rounded-md text-[var(--text-3)] hover:text-[var(--red)] hover:bg-[var(--red-soft)] transition-colors duration-150 opacity-0 group-hover:opacity-100 shrink-0">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button onClick={addCity}
                className="w-full py-2.5 rounded-md border border-dashed border-[var(--border-default)] text-[var(--text-2)] hover:text-[var(--text-1)] hover:border-[var(--border-hover)] hover:bg-[var(--surface-1)] transition-all duration-150 text-xs font-medium flex items-center justify-center gap-1.5">
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                </svg>
                Add City
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-4">
                {monthGroups.map((group) => {
                  ...
                  return (
                    <div key={group.month}>
                      <div className="text-xs font-heading font-semibold text-[var(--text-1)] mb-1.5">{group.month}</div>
                      <div className="grid grid-cols-7 gap-0.5">
                        {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
                          <div key={i} className="text-center text-[9px] text-[var(--text-3)] font-medium py-1">{d}</div>
                        ))}
                        ...
                        {group.dates.map((d) => {
                          ...
                          return (
                            <button
                              ...
                              className={`py-1.5 flex items-center justify-center rounded text-xs transition-all duration-100 ${
                                excluded
              ...
              {excludedDates.length > 0 && (
                <div className="flex items-center justify-between px-2.5 py-2 rounded-md bg-[var(--red-soft)] border border-[var(--red-border)]">
                  <div className="flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-[var(--red)]" />
                    <span className="text-[11px] text-[var(--red)]">{excludedDates.length} blocked</span>
                  </div>
                  <button onClick={() => setExcludedDates([])}
                    className="text-[10px] text-[var(--red)] opacity-70 hover:opacity-100 transition-opacity duration-150 font-medium">
                    Clear
                  </button>
                </div>
              )}
        ...
        <div className="shrink-0 pt-3 mt-3 border-t border-[var(--border-default)] space-y-2">
          ...
          <div className="flex items-center justify-end gap-2">
            <button onClick={() => setOpen(false)}
              className="px-3 py-1.5 rounded-md text-xs text-[var(--text-2)] hover:text-[var(--text-1)] hover:bg-[var(--surface-2)] transition-colors duration-150">
              Cancel
            </button>
            <button onClick={handleSave} disabled={saving || !hasChanges}
              className="px-4 py-1.5 rounded-md text-xs font-medium bg-[var(--blue)] text-white hover:brightness-110 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-150">
              {saving ? (
