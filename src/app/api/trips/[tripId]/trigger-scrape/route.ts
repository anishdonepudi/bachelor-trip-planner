  ...
  try {
    ...
    for (const status of ["in_progress", "queued"] as const) {
      const runsRes = await fetch(
        `https://api.github.com/repos/${githubRepo}/actions/workflows/scrape.yml/runs?status=${status}&per_page=1`,
        {
          headers: {
      ...
      if (runsRes.ok) {
        ...
        if (runsData.total_count > 0) {
          return NextResponse.json({
            success: true,
            message: `Scrape already ${status === "in_progress" ? "in progress" : "queued"}`,
