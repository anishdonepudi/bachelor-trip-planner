...
function FlightCell({
  ...
  if (!cheapest) {
    return <td className="text-right py-2 px-2 align-top"><span className="text-[var(--color-text-tertiary)]">—</span></td>;
  }

  return (
    <td className={`py-2 px-2 align-top ${isSelected ? "bg-[oklch(0.62_0.17_165_/_5%)]" : ""}`}>
      <div className="flex flex-col items-end gap-0.5">
        {/* Price row */}
        <div className="flex items-center gap-1.5">
          {isSelected && (
            <span className="text-[9px] px-1 py-0.5 rounded bg-[oklch(0.62_0.17_165_/_15%)] text-[var(--color-emerald)] border border-[oklch(0.62_0.17_165_/_20%)]">
              Selected
            </span>
          )}
          <a
            ...
            className="text-[var(--color-amber)] hover:text-[var(--color-indigo)] font-mono font-semibold text-sm transition-colors"
          >
            ${cheapest.price}
          </a>
          {hasAlternates && (
        ...
        {expanded && alternates.map((alt, i) => (
          <div key={i} className="mt-1.5 pt-1.5 border-t border-[var(--border)] text-right w-full">
            <a
              href={alt.google_flights_url ?? "#"}
              target="_blank"
              rel="noopener noreferrer"
              className="text-[var(--color-text-primary)] hover:text-[var(--color-indigo)] font-mono text-sm transition-colors"
            >
              ${alt.price}
            </a>
...
export function FlightAllOptions({
  ...
  return (
    <div>
      <button
        onClick={() => setExpanded(!expanded)}
        className="flex items-center gap-2 text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider hover:text-[var(--color-text-primary)] transition-colors"
      >
        <svg
      ...
      {expanded && (
        <div className="mt-3 overflow-x-auto">
          <p className="text-xs text-[var(--color-text-secondary)] mb-2 sm:hidden">Scroll to see all options &rarr;</p>
          <table className="min-w-[600px] w-full text-sm">
            <thead>
              <tr className="text-xs text-[var(--color-text-tertiary)] uppercase tracking-wider">
            ...
            <tbody>
              {cities.map((city) => (
                ...
                >
                  <td className="py-2 px-2 text-[var(--color-text-primary)] align-top">{city}</td>
                  {FLIGHT_CATEGORIES.map((cat) => {
